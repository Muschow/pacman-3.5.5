...
public class GhostScript : CharacterScript
{
    ...
    {
        if (IsOnNode(pacman.Position) && IsOnNode(Position) && recalculate) //every x seconds, if pacman and ghost is on a node, it recalulates shortest path.
        {
            recalculate = false;
        ...
        if (pathCounter < paths.Count)
        {
            ...
            {
                testV = Position.MoveToward(mazeTm.MapToWorld(paths[pathCounter]) + new Vector2(16, 16), delta * Gspeed); //if not, move toward node position
                Position = testV;
